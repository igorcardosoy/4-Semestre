        ...
        <div class="flex gap-3 mt-5 justify-center">
          <div class="alert flex items-center justify-center flex-wrap max-w-min">
            ...
            <label class="input input-bordered flex items-center gap-2 w-96">
              ...
              <c:choose>
                <c:when test="${order.getDescription() != null}">
                  <input name="description" type="text" class="grow" placeholder="Descrição" value="${order.getDescription()}" required="required" />
                </c:when>
                <c:otherwise>
                  <input name="description" type="text" class="grow" placeholder="Descrição" required="required" />
                </c:otherwise>
              </c:choose>
            </label>
            ...
            <label class="input input-bordered flex items-center gap-2 w-96 justify-between">
              ...
              <c:choose>
                <c:when test="${order.getIssueDate() != null}">
                  <input name="issueDate" type="date" required="required" value="${order.getIssueDate()}" />
                </c:when>
                <c:otherwise>
                  <input name="issueDate" type="date" required="required" />
                </c:otherwise>
              </c:choose>
            </label>
            ...
            <label class="input input-bordered flex items-center gap-2 w-96 justify-between">
              ...
              <c:choose>
                <c:when test="${order.getEndDate() != null}">
            ...
            <label class="input input-bordered flex items-center gap-2 w-96">
              ...
              <c:choose>
                <c:when test="${order.getPrice() != null}">
                  <input name="price" type="number" step="0.01" class="grow" placeholder="Preço" value="${order.getPrice()}" required="required" />
                </c:when>
                <c:otherwise>
                  <input name="price" type="number" step="0.01" class="grow" placeholder="Preço" required="required" />
                </c:otherwise>
              </c:choose>
            </label>
            ...
            <label>
              <select id="status-select" class="select select-bordered w-96" required="required" name="status">
                ...
                <c:forEach var="status" items="${statuses}">
                  <c:choose>
                    <c:when test="${order.getStatus() != null && order.getStatus().name() == status.name()}">
                      <option value="${status.name()}" selected>${status.getDescription()}</option>
                    </c:when>
                    <c:otherwise>
                      <option value="${status.name()}">${status.getDescription()}</option>
                    </c:otherwise>
                  </c:choose>
                </c:forEach>
              </select>
            ...
            <label>
                <c:choose>
                  <c:when test="${order.getObservation() != null}">
                  ...
                  <c:otherwise>
                    <textarea class="textarea textarea-bordered w-96 max-h-56" placeholder="Observações" name="observation"></textarea>
                  </c:otherwise>
                </c:choose>
            </label>
